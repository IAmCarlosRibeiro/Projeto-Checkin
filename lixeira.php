...
<body>
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        if (!empty($_POST["start_date"]) && !empty($_POST["end_date"])) {
            ...
            $sqlHours = "SELECT u.nome AS nome, l.cpf AS cpf, SUM(strftime('%s', l.saida) - strftime('%s', l.entrada)) AS duracao_total FROM lixeira AS l INNER JOIN usuarios AS u ON l.cpf = u.cpf WHERE (date(l.entrada) >= :start_date AND date(l.entrada) <= :end_date)";
            ...
            $sql = "SELECT u.nome AS nome, l.cpf AS cpf, l.entrada AS entrada, l.saida AS saida, strftime('%s', l.saida) - strftime('%s', l.entrada) AS duracao FROM lixeira AS l INNER JOIN usuarios AS u ON l.cpf = u.cpf WHERE (date(l.entrada) >= :start_date AND date(l.entrada) <= :end_date)";
